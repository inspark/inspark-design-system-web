(function (window) {
  ...
  if (typeof(exports) !== 'undefined') {
    /* jshint -W020 */
    // require('nvd3');
/*    nv = require('nvd3/src/core.js');
    require('nvd3/src/dom.js');
    require('nvd3/src/interactiveLayer.js');
    require('nvd3/src/tooltip.js');
    require('nvd3/src/utils.js');
    require('nvd3/src/models/axis.js');
    require('nvd3/src/models/candlestickBar.js');
    require('nvd3/src/models/focus.js');
    ...
    require('components/common/nvd3/models/historicalBarChart.js');*/
    /* jshint +W020 */
  }

 angular.module('nvd3', [])

    .directive('nvd3', ['nvd3Utils', '$timeout', function (nvd3Utils) {
      return {
        restrict: 'AE',
        scope: {
          ...
          onBrushExtent: '&?', //callback function that is called with internal scope on focus brush ectent event [optional]
          callback: '&?' //callback function that is called with internal scope on focus brush ectent event [optional]

        },
        link: function (scope, element, attrs) {
          let onBrushExtentCallback = function (params) {
            scope.onBrushExtent({params:params})
          };
...
/*
          d3.select(element[0]).on('click', function (data, index) {        // установка обработчика нажатия мыши
            console.log("D3!!!!!!!!!!!!!!!!!!!!!", data, index);
...
*/
          ...
          scope.api = {
            ...
            clearElement: function () {
              ...
              if (scope.chart && scope.chart.tooltip) {
                if (scope.chart.tooltip.id) {
                  d3.select('#' + scope.chart.tooltip.id()).remove();
                }
                if (scope.chart.interactiveLayer && scope.chart.interactiveLayer.tooltip.id) {
          ...
          function configureEvents(dispatch, options) {
            if (dispatch && options) {
              angular.forEach(dispatch, function (value, key) {
                // dispatch.on('brush',function (e) {
                //   console.log("eeeeeeeee:", e);
                //
                // });
                // console.log("key + '._', options[key]:", key + '._', options[key]);

                if (options[key] === undefined || options[key] === null) {
                  if (scope._config.extended) options[key] = value.on;
                }
                else if (key === 'brush') dispatch.on(key + '._', onBrushExtentCallback);
          ...
          if (scope._config.deepWatchOptions) {
            scope.$watch('options', nvd3Utils.debounce(function (newOptions) {
              ...
              // if (!scope._config.disabled) scope.api.update();
          ...
          /*
           $timeout(function () {
           console.log("scope.options.chart.brushExtent:", scope.options);

           },10000);
           */

          /*
           scope.$watchCollection('options', function(old1, new1){
           console.log("isReady:",scope.options);

           if (scope.onBrushExtent && typeof scope.onBrushExtent() === 'function') {
           console.log("scope.options.chart.brushExtent[0]:", scope.options.chart.brushExtent[0]);

           scope.onBrushExtent()(scope, element);
           }
           });
           */
        }
